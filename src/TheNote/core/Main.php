...
class Main extends PluginBase implements Listener
{
    ...
    //PluginVersion
    public static $version = "7.0.0_ALPHA";
    public static $protokoll = "545";
    public static $mcpeversion = "1.19.30";
    public static $dateversion = "24.09.2022";
    public static $plname = "CoreV7";
    public static $configversion = "7.0.0";
    public static $moduleversion = "7.0.0";
            ...
            if ($config->get("StarterKit") == true) {
                if ($cfg->get("Inventory", false)) {
                    foreach ($cfg->get("Slots", []) as $item) {
                        $result = LegacyStringToItemParser::getInstance()->getMappings($item["id"], $item["damage"], $item["count"]);
    ...
    public function getElevators(Block $block, string $where = "", bool $searchForPrivate = false): int
    {
        if (!$searchForPrivate) {
            ...
            $blocks = [BlockTypeIds::DAYLIGHT_SENSOR];
        }
        $count = 0;
        if ($where === "up") {
    ...
    public function getNextElevator(Block $block, string $where = "", bool $searchForPrivate = false): ?Block
    {
        ...
        $deniedBlocks = [BlockTypeIds::LAVA, BlockTypeIds::WATER];
    ...
    public function configgenerate()
    {
        if (!file_exists($this->getDataFolder() . Main::$cloud . "Money.yml")) {
            ...
            $money->setNested("money.CoreV7.", 1000);
            $money->save();
        }
    }
